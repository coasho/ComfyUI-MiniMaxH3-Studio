...
# --------------------------------------------------------------- 英译中
#
# 反过来这一趟只为「给人读」：反推工作流里英文进提示词、中文供人核对，
# 两边必须说的是同一件事，所以中文由英文译出，而不是让模型分别写两遍。

TRANSLATE_ZH_SYS = (
    "You translate English production notes for a video model into Chinese, "
    "for a human to proofread against the English.\n"
    "Rules:\n"
    "1. Translate each numbered line separately. Output the same numbering, nothing else.\n"
    "2. Plain, concrete Chinese. Same level of detail as the source; "
    "do not add, embellish, summarise or explain.\n"
    "3. Tokens like [[1]] are opaque placeholders. Copy them through exactly as they are, "
    "in the same order. Never translate, renumber, split or drop them.\n"
    # 英文里的 NOT 从句是给 H3 的颜色围栏，人读的那份保留它才对得上
    "4. Keep exclusion clauses. 'dark brown, NOT black, NOT auburn' becomes "
    "「深棕色（不是黑色、不是赤褐色）」—— use the bracket form, never a bare "
    "comma, or the negation attaches to the wrong word.\n"
    "5. Film/animation vocabulary: shot = 镜头, shot size = 景别, camera move = 运镜, "
    "cel shading = 赛璐璐上色, character reference sheet = 设定稿/三视图, "
    "twintails = 双马尾, bangs = 刘海.\n"
)


def translate_to_chinese(lines: list[str], backend: str = "auto") -> list[str]:
    """英文散文 -> 中文。逐行对应返回，长度与输入一致。"""
    items = [str(x or "") for x in lines]
    todo = [(i, t) for i, t in enumerate(items) if t.strip()]
    if not todo:
        return items
    ...
    for n, (idx, src) in enumerate(todo):
        ...
        if len(_PROTECT.findall(out)) < len(stores[n]) or "[[" in out:
            items[idx] = src
            continue
        # 一个中文字都没有 = 压根没翻（模型把英文原样吐回来了），退回原文
